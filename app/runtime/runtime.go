...
func (r *Runtime) runTask(ctx context.Context, cancel context.CancelFunc) error {
	...
	r.audits.Printf("▶️ Starting task: %s", task.Task)
...
func (r *Runtime) runStepWithValidation(sc stepCtx, worker workers.Interface, toolkit map[string]tools.Tool) (bool, string) {
	...
	for attempt := 1; attempt <= sc.MaxAttempts; attempt++ {
		...
		summary += "\nLast 5 records:\n" + storage.RecordListToString(history, 5)
